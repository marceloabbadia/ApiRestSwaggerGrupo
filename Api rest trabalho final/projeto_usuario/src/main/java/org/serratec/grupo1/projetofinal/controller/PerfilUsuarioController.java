...
public class PerfilUsuarioController {
	...
	@PutMapping("/{id}")
	@Operation(summary = "Atualizar perfil de usuário", description = "Atualiza dados de um perfil de usuário", tags = {
			"PerfilUsuario" }, responses = { @ApiResponse(description = "Sucesso", responseCode = "200", content = {
					@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = PerfilUsuarioResponseDto.class))) }),
					@ApiResponse(responseCode = "400", description = "Não autorizado"),
					@ApiResponse(responseCode = "401", description = "Proibido"),
					@ApiResponse(responseCode = "403", description = "Não encontrado"),
					@ApiResponse(responseCode = "500", description = "Erro interno do servidor"), })
	public ResponseEntity<PerfilUsuarioResponseDto> atualizar(@PathVariable Long id,
			@RequestBody PerfilUsuarioRequestDto dto) {
